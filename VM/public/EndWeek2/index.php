	...
	$app->get('/libro/{ident}/', function($ident){
		...
		while($row = $STH->fetch()) {
			if($row['id']==$ident){
				...
				echo "</table>";
				echo "</center>";
				//return "<a href=\"javascript:history.back(-1);\" title=\"Back\">Back</a>";
	...
	$app->match('/search/', function(Request $request) use ($app) {
		...
		if ('POST' == $request->getMethod()) {
			if ($form->isValid()) {
				...
				mysql_select_db('books') or die('Can not select the data base.');
				mysql_query("SET NAMES 'utf8'");
				$query="SELECT `id`, `title` FROM `books` WHERE `title` LIKE '%$search%' or `description` LIKE '%$search%'";
				$result = mysql_query($query) or die('Error!: ' . mysql_error());
				echo "<center><table>\n";
				...
				while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) {
					...
					foreach ($line as $col_value) {
						echo "\t\t<td><a href=\"/bootcamp/wtwo/index.php/libro\">$col_value</a></td>\n";
					}
					echo "\t</tr>\n";
				}
				echo "</table></center>";
